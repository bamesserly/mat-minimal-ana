...
void PlotTotalError(PlotUtils::MnvH1D* hist, std::string method_str ){
  ...
  TCanvas cF ("c4","c4"); 
  hTotalErr->SetTitle(Form("Total Uncertainty (%s); E_{#nu} (MeV)", method_str.c_str()));
  hTotalErr->Draw();
  cF.Print(Form("Enu_TotalUncertainty_%s_%s_%s.png", do_fractional_uncertainty_str.c_str(), do_cov_area_norm_str.c_str(), method_str.c_str()));
}
...
void PlotErrorSummary(PlotUtils::MnvH1D* hist, std::string label){
  ...
  std::string plotname = Form("Enu_ErrorSummary_%s_%s_%s",do_fractional_uncertainty_str.c_str(), do_cov_area_norm_str.c_str(),label.c_str());
...
void PlotVertUniverse(std::string band, unsigned int universe, std::string method_str, PlotUtils::MnvH1D* hist){
  ...
  h1->SetLineStyle(1);
  h1->Draw("hist");
  cF.Print(Form("Enu_%s_band_universe%i_%s.png", band.c_str(), universe+1, method_str.c_str()));
}
...
void PlotCVAndError(PlotUtils::MnvH1D* hist, std::string label){
  PlotUtils::MnvPlotter mnvPlotter(PlotUtils::kCCNuPionIncStyle);
  TCanvas cE ("c1","c1"); 
  hist->GetXaxis()->SetTitle(Form("%s; E_{#nu} (MeV)",label.c_str()));
