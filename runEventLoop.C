...
bool PassesCuts(CVUniverse& univ) {
  ...
  return univ.GetInt("isMinosMatchTrack") == 1 &&
         ...
         univ.GetInt("tdead") <= 1 && univ.GetMuonE() > 2e3;
}
//======================================================================
...
std::map<std::string, std::vector<CVUniverse*> > GetErrorBands(
  ...
  // GENIE
  SystMap genie_systematics = PlotUtils::GetGenieSystematicsMap<CVUniverse>(
      chain, true);  // change that true to a switch on do_nonrespi_tune
...
void runEventLoop() {
  ...
  // Required to make some choices about our model
  ...
  PlotUtils::ChainWrapper* chain =
      makeChainWrapperPtr("playlist.txt", "CCNuPionInc");
